...
class Administrator extends \Model\Admin
{
    ...
    public function get_aministrator_list( $project = null , $list_groups = null , $offset = 0 , $limit = 20 ,$list_type=null )
    {
        ...
        if ( $list_groups || $list_type==2 )
        {
            ...
            $orm_member->driver()->distinct(true)->join('admin_member_group_ids as ids')->on('ids.admin_id', 'm.id');
        }
    ...
    public function search_aministrator_list($project, $list_groups ,$keyword)
    {
        ...
        if ( $list_groups )
        {
            $orm_member->driver()->in('ids.group_id', $list_groups)->distinct(true)->join('admin_member_group_ids as ids')->on('ids.admin_id', 'm.id');
        }

        $orm_member->driver()->select('m.*')->from($orm_member->tablename() .' as m')->order_by('m.id','DESC')->limit(100)->and_where_open()->like('m.username',$keyword)->or_like('m.nickname',$keyword)->and_where_close();
    ...
    public function total_group_count($project=null)
    {
        ...
        return $this->db()->count_records('admin_member_group');
    }
